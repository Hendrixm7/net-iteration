...
namespace DotnetIteration {
   ...
   */
    ...
    public static IEnumerable<string> StringyIndexes (List<string> data) {
      var stringyIndexes = data.Select (item => { return item + " is at index " + data.IndexOf (item); });
    ...
    public static IEnumerable<int> OnlyTheEvenSurvive (List<int> data) {
      var onlyTheEvenSurvive = data.Where (numbers => numbers % 2 == 0);
    ...
    public static IEnumerable<int> OnlyTheEvenIndexedSurvive (List<int> data) {
      var OnlyTheEvenIndexedSurvive = data.Where (number => data.IndexOf (number) % 2 == 0);
    ...
    public static IEnumerable<string> BestMovieOfTheYear (List<Movie> data, int year) {
      var bestMovieOfTheYear = data.Where (movie => movie.Year == year && movie.Score > 90)
    ...
    public static bool EveryoneIsOdd (List<int> data) {
      var everyoneIsOdd = data.Where (number => number % 2 == 1);
      return everyoneIsOdd.Count () == data.Count;
    }
    /*
    ...
    public static string FindTheNeedle (List<string> data) {
      var findTheNeedle = data.Where (word => word.Contains ("needle"));
      return findTheNeedle.First ();
    ...
    public static int FindTheNeedleIndex (List<string> data) {
      var findTheNeedle = data.Where (word => word.Contains ("needle"));
      var needle = findTheNeedle.First ();
    ...
    public static bool SomeoneToLove (List<string> data) {
      var someoneToLove = data.Where (word => word.Length == 4);
      return someoneToLove.Any ();
